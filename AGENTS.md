...
- If user says "commit" or "push", stage intended files only.
- Use direct `git push` by default (not `push.bat` unless user asks).
- Standard push handoff: stage intended files, commit, then direct `git push`.
- In this Codex runtime, always run `git push` with elevated permissions; do not attempt non-elevated first.
