// Auto-generated from admin-ui-src/scripts/scarabs.js. Do not edit this file directly.
...
function renderRows(){const tb=$('rows');tb.innerHTML='';for(const s of state.filtered){const tr=document.createElement('tr');tr.className='row'+(state.selectedId===s.id?' sel':'');const n=s.currentText?.name||'';const g=(s.groupName||'').trim()||'-';const pub=state.tokensByName[n]||'-';const dr=state.draftTokensByScarabId[s.id]||'-';tr.innerHTML='<td><input type="checkbox" data-id="'+s.id+'" '+(state.selectedIds.has(s.id)?'checked':'')+'/></td><td>'+n+'</td><td><span class="chip">'+g+'</span></td><td>'+badge(s.status)+'</td><td><span class="chip">'+pub+'</span></td><td><span class="chip">'+dr+'</span></td>';tr.onclick=(ev)=>{if(ev.target&&ev.target.tagName==='INPUT')return;openEditorById(s.id);};tb.appendChild(tr);}tb.querySelectorAll('input[type="checkbox"]').forEach((cb)=>{cb.onclick=(ev)=>ev.stopPropagation();cb.onchange=(ev)=>toggleRowSelection(cb.dataset.id,ev.target.checked);});syncSelectAllCheckbox();updateBulkActionButtons();status('listStatus','Showing '+state.filtered.length+' scarabs. Selected: '+state.selectedIds.size+'.','ok');}
...
function openEditorCreate(){state.creating=true;state.selected=null;state.selectedId=null;$('name').value='';$('groupName').value='';$('desc').value='';$('mods').value='';$('flavor').value='';$('status').value='active';$('league').value='';$('season').value='';$('meta').textContent='New scarab (not saved yet).';$('tokenPreview').textContent='-';setEditorMode();openModal();$('name').focus();}
...
function editorPayload(){let season=$('season').value.trim()||null;if(!season&&currentScope().seasonId)season=currentScope().seasonId;return{status:$('status').value,name:$('name').value.trim(),groupName:$('groupName').value.trim()||null,description:$('desc').value.trim()||null,modifiers:modArr($('mods').value),flavorText:$('flavor').value.trim()||null,leagueId:$('league').value.trim()||null,seasonId:season};}
...
for(const raw of raws){
  ...
  const body={status:'active',name,groupName:null,description:p.parsed.description||null,modifiers:p.parsed.modifiers||[],flavorText:p.parsed.flavorText||null,leagueId:null,seasonId:sc.seasonId||null};
  let r;
  if(existingId){
