...
var LEAGUE_BASELINE_CACHE_VERSION = "league-baseline-v1";
...
var CANONICAL_SCARAB_LIST = [
  ...
  { name: "Abyss Scarab of Crystals", group: "Abyss" },
  { name: "Abyss Scarab of the Consort", group: "Abyss" },
...
async function ensureIntakeSchema(env) {
  ...
  intakeSchemaReady = (async () => {
    ...
    await env.DB.prepare(
      `CREATE TABLE IF NOT EXISTS league_aggregates (
        league_key TEXT PRIMARY KEY,
        league_kind TEXT NOT NULL,
        session_count INTEGER NOT NULL DEFAULT 0,
        total_consumed INTEGER NOT NULL DEFAULT 0,
        total_trades INTEGER NOT NULL DEFAULT 0,
        total_input REAL NOT NULL DEFAULT 0,
        total_output REAL NOT NULL DEFAULT 0,
        total_input_divine REAL NOT NULL DEFAULT 0,
        total_output_divine REAL NOT NULL DEFAULT 0,
        divine_session_count INTEGER NOT NULL DEFAULT 0,
        total_received INTEGER NOT NULL DEFAULT 0,
        latest_created_at TEXT,
        received_by_scarab TEXT NOT NULL DEFAULT '{}',
        version INTEGER NOT NULL DEFAULT 0,
        updated_at TEXT NOT NULL DEFAULT (CURRENT_TIMESTAMP)
      )`
    ).run();
    await env.DB.prepare(
      `CREATE TABLE IF NOT EXISTS league_weighting_baselines (
        league_key TEXT PRIMARY KEY,
        available INTEGER NOT NULL DEFAULT 0,
        baseline_league_key TEXT,
        included_league_keys TEXT NOT NULL DEFAULT '[]',
        total_consumed INTEGER NOT NULL DEFAULT 0,
        total_received INTEGER NOT NULL DEFAULT 0,
        received_by_scarab TEXT NOT NULL DEFAULT '{}',
    ...
    await env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_league_aggregates_kind_latest ON league_aggregates(league_kind, latest_created_at DESC)").run();
  })().catch((err) => {
...
async function applySessionAggregateDelta(env, sessionRow, direction) {
  ...
  await applyLeagueAggregateDelta(env, sessionRow, dir);
...
function normalizeLeagueStatsForStorage(stats) {
  ...
  return {
    key: String(stats?.key || "challenge:unknown"),
    kind: String(stats?.kind || "challenge"),
    sessionCount: Math.max(0, Math.floor(Number(stats?.sessionCount) || 0)),
    totalConsumed: Math.max(0, Math.floor(Number(stats?.totalConsumed) || 0)),
    totalTrades: Math.max(0, Math.floor(Number(stats?.totalTrades) || 0)),
    totalInput: Math.max(0, Number(stats?.totalInput) || 0),
    totalOutput: Math.max(0, Number(stats?.totalOutput) || 0),
    totalInputDivine: Math.max(0, Number(stats?.totalInputDivine) || 0),
    totalOutputDivine: Math.max(0, Number(stats?.totalOutputDivine) || 0),
    divineSessionCount: Math.max(0, Math.floor(Number(stats?.divineSessionCount) || 0)),
    totalReceived: finalized.totalReceived,
    latestCreatedAt: stats?.latestCreatedAt ? String(stats.latestCreatedAt) : null,
    receivedByScarab,
...
function buildLeagueStatsFromMaterializedRows(rows) {
  const byKey = {};
  for (const row of rows || []) {
    const key = String(row?.league_key || "").trim();
    if (!key)
      continue;
    const receivedByScarab = safeParseJsonMap(row?.received_by_scarab || "{}");
    const stats = normalizeLeagueStatsForStorage({
      key,
      kind: row?.league_kind || normalizeLeagueInfo(key).kind,
      sessionCount: row?.session_count,
      totalConsumed: row?.total_consumed,
      totalTrades: row?.total_trades,
      totalInput: row?.total_input,
      totalOutput: row?.total_output,
      totalInputDivine: row?.total_input_divine,
      totalOutputDivine: row?.total_output_divine,
      divineSessionCount: row?.divine_session_count,
...
async function readMaterializedLeagueStats(env) {
  const { results: rows } = await env.DB.prepare(
    `SELECT league_key, league_kind, session_count, total_consumed, total_trades, total_input, total_output,
            total_input_divine, total_output_divine, divine_session_count, total_received, latest_created_at,
            received_by_scarab, version, updated_at
...
function computeLeagueAggregateFromStats(byKey, rawLeague, cachedBaseline = null) {
  ...
  const targetStats = byKey?.[leagueInfo.key] || createLeagueStats(leagueInfo.key, leagueInfo.kind);
  ...
  return {
    // Target-league metrics are always current-league metrics. The selected
    // weight source may legitimately be the previous league during handover.
    sessionCount: targetStats.sessionCount || 0,
    totalConsumed: targetStats.totalConsumed || 0,
    totalTrades: targetStats.totalTrades || 0,
    totalInput: targetStats.totalInput || 0,
    totalOutput: targetStats.totalOutput || 0,
    totalInputDivine: targetStats.divineSessionCount > 0 ? targetStats.totalInputDivine : null,
    totalOutputDivine: targetStats.divineSessionCount > 0 ? targetStats.totalOutputDivine : null,
    receivedByScarab: targetStats.receivedByScarab || {},
...
function buildHistoricalBaselineFromStats(byKey, currentLeagueKey) {
  ...
  return {
    ...
    includedLeagueKeys: included.map((s) => s.key),
    stats
  };
}
...
async function readCachedLeagueBaseline(env, currentLeagueKey) {
  const row = await env.DB.prepare(
    `SELECT league_key, available, baseline_league_key, included_league_keys,
            total_consumed, total_received, received_by_scarab, weights,
  ...
  return {
    ...
    baselineLeagueKey: row.baseline_league_key || null,
    includedLeagueKeys: safeParseJsonArray(row.included_league_keys || "[]"),
    totalConsumed: Number(row.total_consumed) || 0,
    totalReceived: Number(row.total_received) || 0,
    receivedByScarab: safeParseJsonMap(row.received_by_scarab || "{}"),
    weights: safeParseJsonMap(row.weights || "{}"),
...
async function getOrCreateLeagueBaseline(env, byKey, currentLeagueKey) {
  ...
  const payload = {
    ...
    totalConsumed: baselineStats.totalConsumed,
    totalReceived: baselineStats.totalReceived,
  ...
  await env.DB.prepare(
    `INSERT INTO league_weighting_baselines (
       ...
       total_consumed, total_received, received_by_scarab, weights, version
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(league_key) DO NOTHING`
  ).bind(
    payload.leagueKey,
    payload.available ? 1 : 0,
    payload.baselineLeagueKey,
    JSON.stringify(payload.includedLeagueKeys),
    payload.totalConsumed,
    payload.totalReceived,
    JSON.stringify(payload.receivedByScarab),
...
function pickLeagueModel(leagueInfo, byKey, cachedBaseline = null) {
  ...
  const baselineStats = historicalBaseline ? {
    ...createLeagueStats(
      historicalBaseline.baselineLeagueKey || "historical-baseline",
      "challenge"
    ),
    totalConsumed: historicalBaseline.totalConsumed,
    totalReceived: historicalBaseline.totalReceived,
...
async function readMaterializedLeagueStatsForLeague(env, leagueKey) {
  const row = await env.DB.prepare(
    `SELECT league_key, league_kind, session_count, total_consumed,
            total_trades, total_input, total_output, total_input_divine,
            total_output_divine, divine_session_count, total_received,
            latest_created_at, received_by_scarab, version, updated_at
     FROM league_aggregates
     WHERE league_key = ?`
...
async function replaceMaterializedLeagueAggregates(env, byKey) {
  ...
  for (const statsRaw of Object.values(byKey || {})) {
    const stats = normalizeLeagueStatsForStorage(statsRaw);
    if (!stats.key || stats.sessionCount <= 0)
      continue;
    statements.push(
      env.DB.prepare(
        `INSERT INTO league_aggregates (
          league_key, league_kind, session_count, total_consumed, total_trades, total_input, total_output,
          total_input_divine, total_output_divine, divine_session_count, total_received, latest_created_at,
          received_by_scarab, version, updated_at
        ...
        stats.totalInput,
        stats.totalOutput,
        stats.totalInputDivine,
        stats.totalOutputDivine,
        stats.divineSessionCount,
        stats.totalReceived,
        stats.latestCreatedAt,
        JSON.stringify(stats.receivedByScarab)
      )
    );
  }
...
async function rebuildMaterializedLeagueAggregates(env) {
  ...
  const byKey = buildLeagueStats(rows || []);
  const replaced = await replaceMaterializedLeagueAggregates(env, byKey);
  return {
    ok: true,
    scannedSessions: Array.isArray(rows) ? rows.length : 0,
    leagueCount: replaced.leagueCount
  };
}
...
async function applyLeagueAggregateDelta(env, sessionRow, direction) {
  const dir = Number(direction) >= 0 ? 1 : -1;
  if (dir < 0) {
    return rebuildMaterializedLeagueAggregates(env);
  }
  const leagueInfo = normalizeLeagueInfo(sessionRow?.league_key || sessionRow?.league || "unknown");
  const existing = await env.DB.prepare(
    `SELECT session_count, total_consumed, total_trades, total_input, total_output, total_input_divine,
            total_output_divine, divine_session_count, latest_created_at, received_by_scarab
     FROM league_aggregates
     WHERE league_key = ?`
  ).bind(leagueInfo.key).first();
  const receivedByScarab = safeParseJsonMap(existing?.received_by_scarab || "{}");
  const deltaMap = buildReceivedByScarabMap(parseScarabRows(sessionRow?.scarabs_json || "[]"));
  for (const [name, count] of Object.entries(deltaMap)) {
    const next = (Number(receivedByScarab[name]) || 0) + Number(count || 0);
    if (next > 0)
      receivedByScarab[name] = next;
    else
      delete receivedByScarab[name];
  ...
  await env.DB.prepare(
    ...
    ON CONFLICT(league_key) DO UPDATE SET
      league_kind = excluded.league_kind,
      session_count = excluded.session_count,
      total_consumed = excluded.total_consumed,
      total_trades = excluded.total_trades,
      total_input = excluded.total_input,
      total_output = excluded.total_output,
      total_input_divine = excluded.total_input_divine,
      total_output_divine = excluded.total_output_divine,
      divine_session_count = excluded.divine_session_count,
      total_received = excluded.total_received,
      latest_created_at = excluded.latest_created_at,
      received_by_scarab = excluded.received_by_scarab,
      version = league_aggregates.version + 1,
      updated_at = CURRENT_TIMESTAMP`
  ).bind(
    leagueInfo.key,
    leagueInfo.kind,
    Math.max(0, (Number(existing?.session_count) || 0) + 1),
    Math.max(0, (Number(existing?.total_consumed) || 0) + (Number(sessionRow?.total_consumed) || 0)),
    Math.max(0, (Number(existing?.total_trades) || 0) + (Number(sessionRow?.total_trades) || 0)),
    Math.max(0, (Number(existing?.total_input) || 0) + inputValue),
    Math.max(0, (Number(existing?.total_output) || 0) + outputValue),
    Math.max(0, (Number(existing?.total_input_divine) || 0) + (divineRate > 0 ? inputValue / divineRate : 0)),
    Math.max(0, (Number(existing?.total_output_divine) || 0) + (divineRate > 0 ? outputValue / divineRate : 0)),
    Math.max(0, (Number(existing?.divine_session_count) || 0) + (divineRate > 0 ? 1 : 0)),
    finalized.totalReceived,
    latestCreatedAt,
...
if (path.startsWith("/api/rebuild-league-aggregates") && request.method === "POST") {
      ...
      try {
        ...
        return json({ success: true, ...rebuilt }, 200);
      } catch (e) {
        return json({ error: e.message }, 500);
      }
    }
    ...
    if (path.startsWith("/admin/sessions")) {
      ...
      if (approveMatch && request.method === "POST") {
        ...
        const row = await env.DB.prepare(
          `SELECT id, created_at, intake_state, total_consumed, total_trades, input_value, output_value, divine_rate, scarabs_json, league, league_key,
      ...
      if (request.method === "DELETE") {
        ...
        const row = await env.DB.prepare(
          "SELECT id, created_at, intake_state, total_consumed, total_trades, input_value, output_value, divine_rate, scarabs_json, league, league_key FROM sessions WHERE id = ?"
        ).bind(id).first();
        if (!row)
